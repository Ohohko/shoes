    ...
    <div>
      <button
        className={`
          ...
          rounded-xl text-white transition ${bg}`}
      >
        <span>{text}</span>
      </button>
    </div>
      ...
      <nav className="flex items-center container mx-auto h-full justify-between">
        ...
        <div>
          <ul className="flex items-center space-x-10 text-sm">
            ...
            <li><Link to="/shoes" className="text-gray-400 hover:text-gray-100">Shoes</Link></li>
            <li><Link to="/about" className="text-gray-400 hover:text-gray-100">About Us</Link></li>
            
          </ul>
        </div>
        <div>
          <Button text="Home" bg="bg-gradient-to-r from-purple-500 to-blue-500"/>
        </div>
      </nav>
    </div>
  );
